import cv2
import threading
import queue
import time
import pytesseract
import csv
import os
import subprocess
import numpy as np
from datetime import datetime
from ultralytics import YOLO
from flask import Flask, Response, render_template_string

# ================== CONFIG ==================
MODEL_PATH = "pruned_ncnn_model" 
CSV_FILE = "plate_log.csv"

CAPTURE_WIDTH = 1920
CAPTURE_HEIGHT = 1080
FRAMERATE = 25

INFERENCE_SIZE = 416 
CONF_THRESHOLD = 0.50  

YOLO_INTERVAL = 1.0  # seconds between inferences
OCR_INTERVAL = 2.0   # seconds per plate crop
STREAM_FPS = 10       # MJPEG stream fps

OCR_CONFIG = "--oem 1 --psm 7 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"

PAGE_HTML = """<html><head><title>LPR</title></head>
<body style='background:black;color:white;text-align:center;'>
<h2>Pi 4 LPR</h2><img src='/video_feed' style='max-width:100%;'></body></html>
"""

# Shared state
shared_data = {"boxes": [], "text": "Scanning...", "last_update": 0}
data_lock = threading.Lock()
inference_queue = queue.Queue(maxsize=1)
ocr_queue = queue.Queue(maxsize=1)
display_queue = queue.Queue(maxsize=2)
stop_event = threading.Event()

app = Flask(__name__)
yolo_model = None

# ================== CAMERA THREAD ==================
def capture_worker():
    frame_len = int(CAPTURE_WIDTH * CAPTURE_HEIGHT * 1.5)
    cmd = ["rpicam-vid","-t","0","--inline","--width",str(CAPTURE_WIDTH),
           "--height",str(CAPTURE_HEIGHT),"--framerate",str(FRAMERATE),
           "--codec","yuv420","--nopreview","-o","-"]
    try:
        process = subprocess.Popen(cmd, stdout=subprocess.PIPE, bufsize=10**7)
    except FileNotFoundError:
        cmd[0] = "libcamera-vid"
        process = subprocess.Popen(cmd, stdout=subprocess.PIPE, bufsize=10**7)

    last_infer_time = 0

    while not stop_event.is_set():
        raw_data = process.stdout.read(frame_len)
        if len(raw_data) != frame_len:
            break
        yuv_image = np.frombuffer(raw_data, dtype=np.uint8).reshape((int(CAPTURE_HEIGHT*1.5), CAPTURE_WIDTH))
        frame = cv2.cvtColor(yuv_image, cv2.COLOR_YUV2BGR_I420)

        now = time.time()
        if now - last_infer_time > YOLO_INTERVAL and inference_queue.empty():
            inference_queue.put(frame.copy())
            last_infer_time = now

        if not display_queue.full():
            display_queue.put(frame)

    process.terminate()
    print("[INFO] Capture thread stopped.")

# ================== YOLO THREAD ==================
def yolo_worker():
    global yolo_model
    last_ocr_time = 0
    while not stop_event.is_set():
        try:
            frame = inference_queue.get(timeout=1.0)
        except queue.Empty:
            continue

        # Force YOLO single-threaded
        results = yolo_model(frame, imgsz=INFERENCE_SIZE, conf=CONF_THRESHOLD, verbose=False)

        found_boxes = []
        h, w, _ = frame.shape

        if len(results) > 0 and len(results[0].boxes) > 0:
            for box in results[0].boxes:
                x1, y1, x2, y2 = map(int, box.xyxy[0])
                x1, y1 = max(0,x1), max(0,y1)
                x2, y2 = min(w,x2), min(h,y2)
                found_boxes.append([x1,y1,x2,y2])

                # OCR throttled
                if not ocr_queue.full() and time.time() - last_ocr_time > OCR_INTERVAL:
                    plate_crop = frame[y1:y2, x1:x2]
                    ocr_queue.put(plate_crop)
                    last_ocr_time = time.time()

            with data_lock:
                shared_data['boxes'] = found_boxes
                shared_data['last_update'] = time.time()

# ================== OCR THREAD ==================
def ocr_worker():
    if not os.path.exists(CSV_FILE):
        with open(CSV_FILE, 'w', newline='') as f:
            csv.writer(f).writerow(["Timestamp","Text"])
    last_logged_text = ""
    last_logged_time = 0

    while not stop_event.is_set():
        try:
            plate_img = ocr_queue.get(timeout=1.0)
        except queue.Empty:
            continue

        gray = cv2.cvtColor(plate_img, cv2.COLOR_BGR2GRAY)
        scale = 180 / gray.shape[0]
        gray = cv2.resize(gray, (int(gray.shape[1]*scale),180))
        gray = cv2.GaussianBlur(gray, (3,3),0)
        _, thresh = cv2.threshold(gray,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
        text = pytesseract.image_to_string(thresh, config=OCR_CONFIG)
        clean_text = "".join(text.split()).strip()

        if len(clean_text) >= 3:
            now = time.time()
            with data_lock:
                shared_data['text'] = clean_text
                shared_data['last_update'] = now

            if clean_text != last_logged_text or now - last_logged_time > 5.0:
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                with open(CSV_FILE,'a',newline='') as f:
                    csv.writer(f).writerow([timestamp, clean_text])
                print(f"[LOG] {clean_text}")
                last_logged_text = clean_text
                last_logged_time = now

# ================== MJPEG STREAM ==================
def generate_frames():
    box_color = (0,255,0)
    last_frame_time = 0
    interval = 1.0 / STREAM_FPS

    while True:
        try:
            frame = display_queue.get(timeout=1.0)
            now = time.time()
            if now - last_frame_time < interval:
                continue
            last_frame_time = now

            h,w = frame.shape[:2]
            target_w = 1024
            target_h = int(target_w * (h/w))
            view_frame = cv2.resize(frame,(target_w,target_h))

            scale_x = target_w/w
            scale_y = target_h/h

            with data_lock:
                if time.time() - shared_data['last_update'] < 2.0:
                    boxes = shared_data['boxes']
                    text = shared_data['text']
                    for x1,y1,x2,y2 in boxes:
                        sx1,sy1 = int(x1*scale_x), int(y1*scale_y)
                        sx2,sy2 = int(x2*scale_x), int(y2*scale_y)
                        cv2.rectangle(view_frame,(sx1,sy1),(sx2,sy2),box_color,2)
                        cv2.rectangle(view_frame,(sx1,sy1-25),(sx1+200,sy1),box_color,-1)
                        cv2.putText(view_frame,text,(sx1,sy1-5),
                                    cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,0,0),2)
                else:
                    shared_data['text']="Scanning..."

            ret, buffer = cv2.imencode('.jpg',view_frame)
            yield (b'--frame\r\nContent-Type: image/jpeg\r\n\r\n'+buffer.tobytes()+b'\r\n')

        except queue.Empty: continue
        except Exception: break

@app.route('/')
def index(): return render_template_string(PAGE_HTML)
@app.route('/video_feed')
def video_feed(): return Response(generate_frames(), mimetype='multipart/x-mixed-replace; boundary=frame')

# ================== MAIN ==================
if __name__=="__main__":
    print("[INFO] Loading YOLO Model...")
    yolo_model = YOLO(MODEL_PATH, task='detect')
    dummy = np.zeros((INFERENCE_SIZE, INFERENCE_SIZE,3), dtype=np.uint8)
    yolo_model(dummy, imgsz=INFERENCE_SIZE, verbose=False)
    print("[INFO] YOLO Ready.")

    t1 = threading.Thread(target=capture_worker, daemon=True)
    t2 = threading.Thread(target=yolo_worker, daemon=True)
    t3 = threading.Thread(target=ocr_worker, daemon=True)

    t1.start()
    time.sleep(1)
    t2.start()
    t3.start()

    print("[INFO] Web server running at http://0.0.0.0:5000")
    try:
        app.run(host='0.0.0.0', port=5000, debug=False, threaded=True)
    finally:
        stop_event.set()
        print("[INFO] Shutdown complete.")